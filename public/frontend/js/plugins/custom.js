// fetch('https://crossorigin.me/https://www.metaweather.com/api/location/2487889/')
// .then(result => {
// 	console.log(result);
// 	return result.json();
// })
...
jQuery(document).ready(function($){
	...
	// 		if( email != "" && password != "" ){
	// 			$.ajax({
	// 				url:'https://crossorigin.me/http://127.0.0.1:8000/api/auth/login',
	// 				type:'post',
	...
	$(document).ready(function() {
		...
		var url = 'https://crossorigin.me/http://localhost:8000/api/';
		...
		$('.loginbutton').click(function() {
			...
			$.ajax({
				...
				success: function (res) {
					...
					localStorage.setItem('privilege_id', res.privilege_id);
					...
					//setInterval(localStorage.removeItem('access'), 1000*60*60);
	...
	$("header.slide-bar .close-btn, .header-bar .menu-nav").on("click", function(event) {
		...
		if( $map.length ) {
			$map.gMapResp({
				...
				markers: [{ 
					  ...
					  icon: { 
							image: "public/frontend/js/images/mapicon.png",
							iconsize: [26, 47],
						} ,
					  key: 'key1'
					}],
